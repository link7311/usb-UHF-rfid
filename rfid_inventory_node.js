...
// 指令：INVENTORY，一樣使用 16 進位 bytes（你原註解兩個其實相同）
const CMD_INVENTORY = Buffer.from('BB00220000227E', 'hex');

// ======（選用）開啟 WS + 靜態前端（與之前的 index.html 相容）======
// 設定 ENABLE_WS=1 時生效
...
if (ENABLE_WS) {
  ...
  // 你的 index.html 與 1.png/2.png/3.png 放在 public/
...
// ====== 解析單一 frame（對應 Python parse_frame） ======
...
function parseFrame(fr) {
  ...
  const pc = data.slice(0, 2); // 不特別使用，但保留
...
async function inventoryRound(port, windowMs = 400) {
  // 清空讀緩衝：serialport 沒有 reset_input_buffer，但可丟棄現有資料
  // 我們在開輪時清空暫存 buffer
